 ...
 * @return	None
 */
void InitResponse(uint8_t nDeviceID, XBusOrderID eID, uint8_t Data1, uint8_t Data2)
{
 ...
 * @return	None
 */
void DoSet(uint8_t nParamIdx)
{
	// response packet
	if (nParamIdx == XBus_ParamIdx_Reversed)
	{
		CommitConfig();
		InitResponse(1, XBus_Order_ParamWrite, 0, XBus_ParamIdx_Reversed);
	}
	else
	{
		InitResponse(1, XBus_Order_Unsupported, 0, 0);
	}
}

void DoIDChange(uint8_t nCurrentID, uint8_t nNewID)
{
  UpdateFunctionChannel(nCurrentID, nNewID);
  CommitConfig();
  InitResponse(nCurrentID, XBus_Order_ID, nNewID, 0);
}
...
void PerformCommand(const uint8_t* pXBusPacket)
{
	...
	{
		case XBus_Order_Mode: InitResponse(1, XBus_Order_Mode, 2, 0); break;
		case XBus_Order_ID: DoIDChange(pCommand->DeviceID, pCommand->Data1); break;
		case XBus_Order_ParamWrite: DoSet(pCommand->Data2_Crc); break;

		default: InitResponse(1, XBus_Order_Unsupported, 0, 0); break;
	}
...
void PerformStatus(const uint8_t* pXBusPacket)
{
	...
	switch (pCommand->Order)
	{
		case XBus_Order_Version: InitResponse(1, XBus_Order_Version, 1, 0); break;
		case XBus_Order_Product: InitResponse(1, XBus_Order_Product, 0, 0); break;

		default:  InitResponse(1, XBus_Order_Unsupported, 0, 0);
	}
